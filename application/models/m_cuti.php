...
class M_cuti extends CI_Model
{
    ...
    public function get_all_cuti_diterima()
    {
        $hasil = $this->db->query("SELECT * FROM cuti JOIN user ON cuti.id_user = user.id_user JOIN user_detail ON user.id_user_detail = user_detail.id_user_detail WHERE cuti.id_status_cuti = '2' ORDER BY user_detail.nama_lengkap ASC");
        return $hasil;
    }
    ...
    public function update_cuti($id_cuti, $bukti_pembayaran)
    {
        $this->db->trans_start();
        $this->db->query("UPDATE cuti SET bukti_pembayaran='$bukti_pembayaran' WHERE id_cuti='$id_cuti'");
        $this->db->trans_complete();
        if($this->db->trans_status()==true)
            return true;
        else
    ...
    public function update_cuti_bak($perihal_cuti, $tgl_diajukan, $mulai, $id_cuti, $surat_ortu, $surat_kebenaran)
    {
        $this->db->trans_start();
        $this->db->query("UPDATE cuti SET perihal_cuti='$perihal_cuti', tgl_diajukan='$tgl_diajukan', mulai='$mulai', surat_ortu='$surat_ortu', surat_kebenaran='$surat_kebenaran' WHERE id_cuti='$id_cuti'");
        $this->db->trans_complete();
        if($this->db->trans_status()==true)
            return true;
        else
